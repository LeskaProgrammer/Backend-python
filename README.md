...
- Авторизация по токену (`/api/auth/token/`) и права доступа (только автор может менять свои заметки/комментарии)
- Полный набор API-тестов на ручки и контроллеры (`backend/notes/tests.py`)
- Обновлённый запуск через Docker Compose для backend
...
API будет доступен на `http://localhost:8000`, Swagger — `http://localhost:8000/api/docs/`.
